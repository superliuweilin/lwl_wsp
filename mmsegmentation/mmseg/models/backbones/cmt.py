...
class CMT(BaseModule):
    ...
    def forward_features(self, x):
        ...
        B, N, C = x.shape
        f1 = x.permute(0, 2, 1).reshape(B, C, H, W)
        outs.append(f1)
        # print(f1.shape)
        ...
        B, N, C = x.shape
        f2 = x.permute(0, 2, 1).reshape(B, C, H, W)
        # print(f2.shape)
        outs.append(f2)
        ...
        B, N, C = x.shape
        f3 = x.permute(0, 2, 1).reshape(B, C, H, W)
        outs.append(f3)
        # print(f3.shape)
        ...
        B, N, C = x.shape
        f4 = x.permute(0, 2, 1).reshape(B, C, H, W)
        f4 = self._bn(f4)
        f4 = self._swish(f4)
        outs.append(f4)
        # print(f4.shape)
...
def _create_cmt_model(pretrained=False, distilled=False, **kwargs):
    ...
    model = CMT(img_size=512, num_classes=num_classes, representation_size=repr_size, **kwargs)
